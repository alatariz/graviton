      ...
      <nav class="nav-center-links">
        ...
        <div class="nav-item has-dropdown ${activePage === 'product' ? 'active' : ''}">
          ...
          <div class="dropdown-menu">
            ...
            <a href="/#evidence" class="dropdown-item">Why Graviton</a>
            <a href="/#how-it-works" class="dropdown-item">How it works</a>
            <a href="/#demo" class="dropdown-item">Demo</a>
            <a href="/#solution" class="dropdown-item">Architecture</a>
            <a href="/docs.html#skill-matrix" class="dropdown-item item-pro">
              <span class="pro-dot"></span>
              <span>Graviton Pro</span>
            </a>
          </div>
        </div>
        ...
        <div class="nav-item has-dropdown">
          ...
          <div class="dropdown-menu">
            <a href="/#action" class="dropdown-item">Benchmarks</a>
            <a href="/docs.html#skill-matrix" class="dropdown-item">Skill Matrix</a>
            <a href="https://github.com/alatariz/graviton" target="_blank" rel="noopener noreferrer" class="dropdown-item">Changelog</a>
          </div>
        </div>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* ALL PAGE STAGES SCROLL SNAP CONFIGURATION */
    .hero-top-stage,
    ...
    <section id="solution" class="section-stage-solution">
      <div class="section-solution-grid">
        <div class="solution-left slide-reveal-left">
          ...
          <h2>Precision Context &amp;<br><em>Autonomous Relay</em></h2>
          <p>
            Graviton is engineered around high-precision context optimization and autonomous execution designed to eliminate agent context exhaustion.
          </p>
          <div style="margin-top: 1.5rem; display: flex; gap: 0.8rem; flex-wrap: wrap;">
            <span class="res-badge" style="font-size: 0.75rem; padding: 0.35rem 0.75rem;">⚡ Terminal Log Filter</span>
            <span class="res-badge" style="font-size: 0.75rem; padding: 0.35rem 0.75rem;">📦 JSON Array Compression</span>
            <span class="res-badge" style="font-size: 0.75rem; padding: 0.35rem 0.75rem;">✂️ Native-First Execution</span>
            <span class="res-badge" style="font-size: 0.75rem; padding: 0.35rem 0.75rem;">🎯 Precision Synthesis</span>
            <span class="res-badge" style="font-size: 0.75rem; padding: 0.35rem 0.75rem;">🚀 Autonomous Relay</span>
          </div>
        </div>

        <div class="specs-table-box slide-reveal-right">
          <div class="specs-table-title">GRAVITON CORE SPECIFICATIONS</div>

          <div class="spec-table-row">
            <span class="spec-label">LOG FILTER</span>
            <span class="spec-val">Terminal noise interception. Strips progress bars, compilations &amp; isolates tracebacks.</span>
          </div>
          <div class="spec-table-row">
            <span class="spec-label">JSON SQUASHING</span>
            <span class="spec-val" style="color: var(--aqua);">JSON compression. Truncates oversized array floods while preserving schemas (&gt;85% token save).</span>
          </div>
          <div class="spec-table-row">
            <span class="spec-label">NATIVE-FIRST</span>
            <span class="spec-val">Strictly enforces standard library runtime APIs over unnecessary third-party packages.</span>
          </div>
          <div class="spec-table-row">
            <span class="spec-label">PRECISION SYNTHESIS</span>
            <span class="spec-val">Maximizes Signal-to-Noise ratio by eliminating fluff and expanding technical specifications.</span>
          </div>
          <div class="spec-table-row">
            <span class="spec-label">PRECISION MODE</span>
            <span class="spec-val" style="color: #38bdf8;">Standard Engine: Ultra-fast task clarification &amp; log isolation with zero token bloat.</span>
          </div>
          <div class="spec-table-row">
            <span class="spec-label">DEEP ARCHITECTURE</span>
            <span class="spec-val" style="color: var(--aqua);">Architect Mode (--deep): Modular system decomposition with step-by-step implementation blueprints.</span>
          </div>
          <div class="spec-table-row">
            <span class="spec-label">AUTO-ALLOW</span>
            <span class="spec-val">Autonomous execution relay with programmatic affirmative stream for Antigravity.</span>
          </div>
        </div>
      </div>
    </section>
      ...
      <div class="section-action-grid">
        ...
        <div class="action-terminal-card slide-reveal-right">
          <div class="action-tabs-bar">
            ...
            <button class="action-tab-item" onclick="switchActionTab('json', this)">JSON Compression</button>
            <button class="action-tab-item" onclick="switchActionTab('precision', this)">Precision Prompt</button>
  ...
  <script>
    ...
    const ACTION_PRESETS = {
      ...
      json: {
        ...
        clean: "{\\n  \\\"status\\\": \\\"ok\\\",\\n  \\\"users\\\": [\\n    { \\\"id\\\": 1, \\\"name\\\": \\\"Alice\\\", \\\"role\\\": \\\"admin\\\", \\\"token\\\": \\\"[[REDACTED]]...\\\" },\\n    \\\"[... GRAVITON CORE: 148 items truncated ...]\\\",\\n    { \\\"id\\\": 150, \\\"name\\\": \\\"Zoe\\\", \\\"role\\\": \\\"viewer\\\", \\\"token\\\": \\\"[[REDACTED]]...\\\" }\\n  ]\\n}"
      },
      precision: {
        rawLines: '74 TOKENS (RAW FLUFF)',
        cleanLines: 'PRECISION SYNTHESIZED',
        ...
        clean: "[CWD: /workspace/project]\\n\\n<graviton_plan>: Implement defensive null-checks natively using optional chaining and nullish coalescing to prevent TypeError exceptions.\\n\\n[TARGET SPECIFICATIONS]: Fix verifyUser in auth.js. Ensure strict null-safety on req.user and return fallback null when session is missing.\\n\\nexport function verify(req) {\\n  return req?.user?.id ?? null;\\n}"
      },
      architect: {
        rawLines: '45 TOKENS (VAGUE)',
        cleanLines: 'ARCHITECT MODE (--deep)',
        raw: "Tolong buatkan sistem webhook payment gateway yang scalable dan aman dari double spending.",
        clean: "[CWD: /workspace/project]\\n\\n<graviton_plan>: Implement idempotent webhook processing with timing-safe HMAC verification and distributed lock deduplication.\\n\\n[TARGET SPECIFICATIONS]: Build idempotent payment webhook consumer. Verify signature with crypto.timingSafeEqual, acquire distributed lock, execute atomic ledger transaction, and return 200 OK."
      },
      git: {
        rawLines: '64 LINES',
    ...
    // =========================================================
    // SMART PAGE-BY-PAGE SMOOTH SNAP CONTROLLER
    // Direct transition to next/previous stage on wheel or keys
    // =========================================================
    (function initPageSnapper() {
      ...
      function getStages() {
        return stageSelectors.map(function(sel) {
      ...
      function getCurrentIndex(stages) {
        ...
        const probe = scrollY + 58 + 40; // probe just under navbar
        for (let i = stages.length - 1; i >= 0; i--) {
          if (probe >= stages[i].offsetTop) {
            return i;
          }
        }
        return 0;
      ...
      function scrollToStageIndex(stages, idx) {
        ...
        const targetTop = target.getBoundingClientRect().top + window.scrollY - 58;
      ...
      window.addEventListener('wheel', function(e) {
        ...
        // Check if cursor is inside an interactive scrollable area
        let node = e.target;
        while (node && node !== document.body && node !== document.documentElement) {
          if (node.classList && (
            node.classList.contains('search-results-list') ||
            node.classList.contains('interactive-box') ||
        ...
        if (currentStage) {
          ...
          if (delta > 0) {
            // If stage is taller than screen and user hasn't scrolled to its bottom, scroll inside it
            ...
            // Scrolling up: if not at top of current stage, scroll inside it
            if (rect.top < 58 - 50) {
              return;
            }
            if (currentIdx > 0) {
...
# Filter terminal command output:
